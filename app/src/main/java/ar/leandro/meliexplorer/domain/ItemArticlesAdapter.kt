...
class ItemArticlesAdapter(val articles: List<Article>) : RecyclerView.Adapter<ItemArticlesAdapter.ItemArticlesHolder>() {
    ...
    class ItemArticlesHolder(val binding: ItemArticleBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(articles: Article) {
            ...
            Picasso.get().load(articles.thumbnail).into(binding.ivPicture)
        }
    }
